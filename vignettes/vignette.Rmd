...
*PCAmatchR* matches cases to controls based on genotype principle components (PC). In order to produce more genetically similar matches, a weighted Mahalanobis distance metric (Kidd et al. (1987) <DOI:10.1016/0031-3203(87)90066-5>) is used to determine matches. Weights are equal to the percent variance explained by each PC.
...
```
*PCAmatchR* depends on the *optmatch* package which must be manually installed and from CRAN:
```{r eval=FALSE}
...
match_maker_output<- match_maker(PC = pcs,
                                 ...
                                 case_control="case",
                                 num_controls = 2,
                                 num_PCs = dim(covariate_data)[1])
```
Derived matches are contained with the `matches` object. The weighted Mahalanobis distance metric between case and control pairs is detailed within the `match_distance` variable:
```{r eval=FALSE}
